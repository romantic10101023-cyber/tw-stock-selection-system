...
test('429 and 503 responses are retried before a real official response is returned', async () => {
  ...
  const delays = [];
  const result = await fetchOfficial('https://official.example/market', {
...
test('repeated official endpoint timeouts open a circuit and fail fast', async () => {
  let calls=0;
  const fetchImpl=async(_url,{signal})=>new Promise((_resolve,reject)=>{calls++;signal.addEventListener('abort',()=>reject(Object.assign(new Error('timed out'),{name:'AbortError'})),{once:true});});
  for(let index=0;index<3;index++) await assert.rejects(fetchOfficial('https://circuit.example/history',{fetchImpl,timeoutMs:2,attempts:1,minIntervalMs:0,sleepImpl:async()=>{},logger:quiet}));
  await assert.rejects(fetchOfficial('https://circuit.example/history',{fetchImpl,timeoutMs:2,attempts:1,minIntervalMs:0,sleepImpl:async()=>{},logger:quiet}),/circuit open/);

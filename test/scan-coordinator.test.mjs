...
test('coordinator backs off when a deployment lease is already held',async()=>{let calls=0;const started=Date.now();const coordinator=createScanCoordinator({runBatch:async()=>{calls++;return calls===1?{alreadyRunning:true}:{queueComplete:true};},retryDelayMs:20,logger:{error(){}}});coordinator.start();await coordinator.done();assert.equal(calls,2);assert.ok(Date.now()-started>=15);});

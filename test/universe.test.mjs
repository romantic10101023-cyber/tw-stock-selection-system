...
import { classifyQuotes, parseSecurityMaster } from '../src/security-master-provider.mjs';
...
test('official security master excludes ETFs, financials and construction before the candidate universe', () => {
  ...
  const result = filterUniverse(classifyQuotes(raw, master));
...
test('excluded securities never enter the persistent history queue', () => {
  const filtered = filterUniverse(classifyQuotes([quote('0050','元大台灣50'),quote('2881','富邦金'),quote('2501','國建'),quote('1216','統一'),quote('6488','環球晶','tpex')], master));

...
export async function runProductionBatch({dataDir=process.env.DATA_DIR??'data',asOf=process.env.MARKET_DATE??new Date().toISOString().slice(0,10),logger=console,refreshUniverse=false}={}) {
  ...
  const factors=batch.queueComplete?await loadOfficialFactorsDetailed(universe.included.map(s=>s.code)):{factors:{},sources:{status:'deferred_until_history_queue_complete'}};
  const stocks=enrichBatch(universe.included,factors.factors);
